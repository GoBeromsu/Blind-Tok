import {roomAddUser, checkData, removeUserList, show_r, createRoom, updateRoom, getRoomData_name_user} from "@utils/ChatRoomUtils";
import {getUserRoomList, removeRoomList, updateRoomList, show_u, userAddRoom} from "@utils/ChatUserUtils";
...
// getRoomData_name_user : 방 하나를 가공하는 함수
...
// data.userlist.map : 주어진 유저리스트 마다 반복한다.
...
// 유저는 배열 형태로 전달된다.
// 배열에 있는 유저마다 처리를 해준다.
// roomAddUser : ChatRoomUtils에 있는 해당 방의 데이터에 userlist가 존재하는데 여기에 유저를 추가한다.
// userAddRoom : ChatUserUils에 있는 유저 데이터에 roomlist가 있는데 여기에 해당 방을 추가한다.
